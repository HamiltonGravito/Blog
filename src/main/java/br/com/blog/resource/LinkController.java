...
public class LinkController {
	...
	@GetMapping("/{id}")
	public ResponseEntity<List<Link>> getLinkPorIdPost(@PathVariable Long id){
		List<Link> linksId = linkService.listarLinkIdPost(id);
		for (Link link : linksId) {
			System.out.println(link);
		}
		return ResponseEntity.status(HttpStatus.ACCEPTED).body(linksId);
	}
